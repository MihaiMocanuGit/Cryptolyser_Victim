...
#pragma pack(0)
        ...
        ssize_t receivedLength =
            recvfrom(sock, &packet, sizeof(packet), 0, (struct sockaddr *) &sender_addr, &sender_len);
        if (receivedLength < 0) {
        ...
        // This will need further investigation, the compiler is actually free to reorder this sequence.
        // Find a way to stop it, volatile might not do the trick. Check compiler/memory barriers.
        volatile struct timespec inbound_time;
        clock_gettime(CLOCK_PROCESS_CPUTIME_ID, (struct timespec *) &inbound_time);

        int ciphertext_len;
        volatile unsigned char *ciphertext = aes_encrypt(en, plaintext, plaintext_len, &ciphertext_len);
...
#pragma pack(1)
        ...
        } timing = {.inbound_sec = htobe64(inbound_time.tv_sec), .inbound_nsec = htobe64(inbound_time
                                                                                             .tv_nsec), .outbound_sec = htobe64(
            outbound_time.tv_sec), .outbound_nsec = htobe64(outbound_time.tv_nsec)};
#pragma pack(0)

        // Send time data back to the sender0
        if (sendto(sock, &timing, sizeof(timing), 0, (struct sockaddr *) &sender_addr, sizeof(sender_addr)) < 0) {
